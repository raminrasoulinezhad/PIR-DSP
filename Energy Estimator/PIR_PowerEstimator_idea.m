function [Power_Rimp, Power_Ours] = PIR_PowerEstimator(Power_Rimp, Power_Ours, functionality, index, N, kernel, stride, MAC)
    ...
    %PDSP48E2MAC = PMAC;
    %%%%%%%%%%%%%%%%%%
    % We should apply something for our approach as well like that to
    % decrease the energies per precision
    precisions = [9,4,2];
    CSF_per_presicion = precisions(Depth+1)/9;
    Pf = (2.5 * Pf) / CSF_per_presicion;
    %
    ...
    switch functionality
        case '3D' 
            if ((stride == 1) && (kernel == 3))
                Power_Rimp(1) = Power_Rimp(1) + (((Pb/(kernel*kernel)) + Psr + Pf   +   (Pb/(index*index)) + Pf)*CSF_per_presicion         + PDSP48E2MAC) * MAC;
                if (Stream)
                    Power_Ours(1) = Power_Ours(1) + (( Pb + 2*Pc ) / (RFs*kernel*kernel) + PSRe    +     Pb/(index*index) + PRFe       + PMAC) * MAC;
                else
                    Power_Ours(1) = Power_Ours(1) + (((Pb/(kernel*kernel)) + Psr + Pf   +   (Pb/(index*index)) + Pf)*CSF_per_presicion         + PMAC) * MAC;
                end
        ...
        case 'DW'
            if ((stride == 1) && (kernel == 3))
                Power_Rimp(2) = Power_Rimp(2) + (((Pb/(kernel*kernel)) + Psr + Pf   +   (Pb/(index*index)) + Pf)*CSF_per_presicion         + PDSP48E2MAC) * MAC;
                if (Stream)
                    Power_Ours(2) = Power_Ours(2) + (( Pb + 2*Pc ) / (kernel*kernel) + PSRe    +     PRFe + Pb/(index*index)       + PMAC) * MAC;
                else
                    Power_Ours(2) = Power_Ours(2) + (((Pb/(kernel*kernel)) + Psr + Pf   +   (Pb/(index*index)) + Pf)*CSF_per_presicion       + PMAC) * MAC;
                end
        ...
        case 'PW'
                Power_Rimp(3) = Power_Rimp(3) + ((Pb/N + Psr + Pf      +     (Pb/(index*index)) + Pf)*CSF_per_presicion         + PDSP48E2MAC) * MAC;
                if (Stream)
                    Power_Ours(3) = Power_Ours(3) + ((Pb/N + Pc)/RFs + PSRe      +     (Pb/(index*index)) + PRFe       + PMAC) * MAC;
                else
                    Power_Ours(3) = Power_Ours(3) + ((Pb/N + Psr + Pf      +     (Pb/(index*index)) + Pf)*CSF_per_presicion       + PMAC) * MAC;
                end
                
        case 'FC'
                Power_Rimp(4) = Power_Rimp(4) + (((Pb/(N)) + Pf + Pb + Pf)*CSF_per_presicion         + PDSP48E2MAC) * MAC;
                if (Stream)
                    Power_Ours(4) = Power_Ours(4) + ((Pb/(N)) + PSRe + Pb + PRFe       + PMAC) * MAC;
                else
                    Power_Ours(4) = Power_Ours(4) + (((Pb/(N)) + Pf + Pb + Pf)*CSF_per_presicion       + PMAC) * MAC;
                end
